...
## 可选 X/Twitter API 伴随路径

`social-push` 的主路径仍然是浏览器自动化发帖。对于不适合走浏览器的
X/Twitter 工作流，可以把 [TweetClaw](https://github.com/Xquik-dev/tweetclaw)
作为独立的 OpenClaw 插件配合使用，而不是替换本仓库的发布矩阵。

适合交给 TweetClaw 的任务包括搜索推文、搜索推文回复、导出关注者、用户查询、
媒体上传与下载、私信、账号监控、webhook 提醒和抽奖抽取。需要从 OpenClaw
里直接发布推文或回复时，也可以用 TweetClaw 走明确的凭据配置和审批边界。
...
- ClawHub: [`xquik/tweetclaw`](https://clawhub.ai/xquik/tweetclaw)
